...
public class Vision extends SubsystemBase {
    ...
    private void setSmartDashboardValues() {
        ...
        SmartDashboard.putNumber(camera.getName() + "estimated field relative robot position", getFieldRelativePose().estimatedPose.getRotation().toRotation2d().getDegrees());
        SmartDashboard.putNumber(camera.getName() + "pidgeon readings ", drivetrain.getPigeon2().getYaw().getValueAsDouble());

        SmartDashboard.putNumber(camera.getName() + " closest field rel yaw", tagToCamera.getRotation().getAngle());
        SmartDashboard.putNumber(camera.getName() + " closest field rel X", tagToCamera.getX());
        SmartDashboard.putNumber(camera.getName() + " closest field rel Y", tagToCamera.getY());
